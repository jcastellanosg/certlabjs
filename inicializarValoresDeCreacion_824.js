(function () {
    ...
    function imprimir() {
        ...
        $('#' + capitalizar(tipo) + '_forcod').html('CAL-P05-F3');
        $('#' + capitalizar(tipo) + '_forver').html('5');
        $('#' + capitalizar(tipo) + '_forvig').html('05/02/2015');



        orden = acciones.recoverValor['oculto']($('#editar_o_id'))[0];
        ...
        proveedor = acciones.recoverValor['oculto']($('#editar_p_nombre'))[0];
        nit = acciones.recoverValor['oculto']($('#editar_p_nit'))[0];
        direccion = acciones.recoverValor['oculto']($('#editar_p_direccion'))[0];
        telefono = acciones.recoverValor['oculto']($('#editar_p_telefono'))[0];
        email = acciones.recoverValor['oculto']($('#editar_p_email'))[0];

        tabla = ['<tr><td>Nro Orden </td><td>Nro Cotizacion</td><td>Fecha creacion </td><td>Proveedor </td></tr>',
            '<tr><td>' + orden + '</td><td>' + cotizacion + '</td><td>' + fechacreacion + '</td><td>' + proveedor + '</td></tr>',
            '<tr><td>Nit</td><td>Direccion</td><td>telefono</td><td>email</td></tr>',
            '<tr><td>' + nit + '</td><td>' + direccion + '</td><td>' + telefono + '</td><td>' + email + '</td></tr>'
        ...
        textocotizacion = '<h3><strong>Orden de Servicio</strong> </h3>';
