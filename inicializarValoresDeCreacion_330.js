(function () {
    ...
    $("button[data-facturacion-accion='addrowdt_row']").html('<i data-facturacion-tipo="accion" data-facturacion-accion="addrowdt_row" class="fa fa-check"></i>Add Producto Ok');
    $(".modal-title").text("Crear Orden de Compra");
    ...
    function calcularTotaldeCotizacion() {
        var parcial,
            total = 0;
        for (var i = 0; i < cotizaciondetalle.data.length; i++) {
            parcial = cotizaciondetalle.data[i].d_precio * cotizaciondetalle.data[i].d_cantidad;
            if (parcial <= 0) {
                total = 0;
            } else {
                total += parcial;
    ...
    function guardarcotizacion(pantalla) {
        var rowvalues = {},
            url = window.urlcrear.replace('listar.crear', 'listar.crearordendecompra'),
    ...
    function validateCotizacion(rowvalues) {
        var resultado = true;
        if (isNaN(rowvalues.c_proveedor_id) || parseInt(rowvalues.c_proveedor_id) <= 0) {
            displayerror("warning", 'Seleccione un proveedor valido');
            resultado = false;
        }
        if (isNaN(rowvalues.c_precio) || parseInt(rowvalues.c_proveedor_id) <= 0) {
            displayerror("warning", 'Seleccione un proveedor valido');
            resultado = false;
        }
        if(isNaN(rowvalues.c_diasentrega) || parseInt(rowvalues.c_diasentrega) <= 0){
            displayerror("warning", 'Seleccione un estimado de dias de entrega');
            resultado = false;
        }
        if(rowvalues.c_condicionesdenegociacion.trim() == ''){
            displayerror("warning", 'No hay condiciones de negociacion');
            resultado = false;
        }
        return resultado;
    }
