(function () {
    ...
    $(".modal-title").text("Crear Requisicion");
    ...
    function guardarcotizacion(pantalla) {
        var rowvalues = {},
            url = window.urlcrear.replace('listar.crear', 'listar.crearrequisicion'),
    ...
    function validateCotizacion(rowvalues) {
        ...
        if(rowvalues.c_descripcion.trim() == ''){
            displayerror("warning", 'Escriba una descripco del pedido');
            resultado = false;
        }
        if(isNaN(rowvalues.c_solicitadopor) || !(parseInt(rowvalues.c_solicitadopor)>0)){
            displayerror("warning", 'Falta el solicitante');
            resultado = false;
        }
        if (cotizaciondetalle.data.length <=0){
            displayerror("warning", 'No ha solicitado productos');
            resultado = false;
        }
        return resultado;
    }
