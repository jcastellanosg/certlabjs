    ...
    function guardarcotizacion(pantalla) {
        ...
        if (validateOperacion(rowvalues['c_estado_id'])) {
            if (parseInt(rowvalues['c_estado_id']) === 1) {
                url = window.urlcrear.replace('listar.crear', 'listar.crearCotizacionAutorizada');
            } else {
...
//                rowvalues['d_producto_id_txt'] = rowvalues['p_nombre'];
    ...
    function imprimir() {
        ...
        $('#' + capitalizar(tipo) + '_forcod').html('CAL-P05-F2');
        ...
        var condiciondepago = $("#"+ tipo + "_c_condicionesdepago").val();
        $("#" + capitalizar(tipo) + "imprimircondicionesdepago").text(condiciondepago);
        window.print();
    }
